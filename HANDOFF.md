...
- **停止→再生で波形範囲が消える問題を修正**: `player.js` の停止処理が `currentIndex=-1` にした後、次の再生で常にキュー先頭曲へ戻していたため、範囲選択曲と再生再開曲がズレて範囲が消えていた。停止前の `lastStoppedIndex` を保持し、停止直後の再生は同じ曲から再開するよう修正。HTML参照を `player.js?v=20260624a` に更新しXserver本番へ反映済み。本番音楽素材ページで Moment の範囲選択→停止→再生後も範囲UIが残ることを確認。
