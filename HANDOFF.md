...
- **制作ツールボックス緊急復旧**: 直前の「音楽手帖プレビュー波形と制作ツール同期」で `player-panel.js` に入れた範囲共有処理が制作ツールボックス本体のクリック反応/波形表示を不安定化したため、`player-panel.js` だけ安定版へロールバック。HTML参照は `player-panel.js?v=20260624h` に更新しXserver本番へ反映済み。本番スモークで制作ツールボックスが開き、波形プレビューCanvas/タブ群が表示され、console errorなしを確認。同期機能は `player-panel.js` に干渉しない別設計で再実装すること。
