#!/usr/bin/env bash
# H/MIX — お気に入り常駐ハブ＋飛ぶハート デプロイ: fav-modal.js/css + ?vバンプ参照ページ
set -u
cd "$(dirname "$0")/.." || exit 1
source ./.env.ftp
ASSETS=( "assets/js/fav-modal.js" "assets/css/fav-modal.css" )
mapfile -t PAGES < <(grep -rlE 'fav-modal\.(js|css)' --include='*.html' . 2>/dev/null | sed 's#^\./##' | grep -vE 'backups/|_archive|archive-restored|_mockup|node_modules|newtop' | sort -u)
